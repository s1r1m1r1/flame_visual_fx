...
class PolygonFireDecorator extends VFXDecorator {
  ...
  void apply(void Function(ui.Canvas) draw, ui.Canvas canvas, [Component? component]) {
    ...
    for (final p in particles) {
      ...
      ui.Color color;
      if (progress < 0.3) {
        color = ui.Color.lerp(const ui.Color(0xFFFFFF00),
            const ui.Color(0xFFFF9900), progress / 0.3)!;
      } else if (progress < 0.7) {
        color = ui.Color.lerp(const ui.Color(0xFFFF9900),
            const ui.Color(0xFFFF0000), (progress - 0.3) / 0.4)!;
      } else {
      ...
      final blur = _blurCache.putIfAbsent(
        quantizedSizeKey,
        () => ui.MaskFilter.blur(
            ui.BlurStyle.normal, quantizedSizeKey / 1.66 * 0.5),
      );
