...
void main() {
  runApp(
    const MaterialApp(
      ...
      home: DissolveBenchmarkScreen(),
    ),
  );
}

class DissolveBenchmark extends FlameGame {
  DissolveBenchmark() : super(world: BenchmarkWorld());
  ...
  Future<void> onLoad() async {
    ...
    effects.addAll([
      ...
      MapEntry(
        'Dissolve Decorator (CPU)',
        (Ptero p) => p.decorator.addLast(
          DissolveDecorator(
            component: p,
            type: DissolveType.random,
            noiseWeight: 0.5,
            mask: GridDissolveMask(),
          ),
        ),
      ),
      MapEntry(
        'Shader Dissolve Organic (GPU)',
        (Ptero p) => p.decorator.addLast(
...
class BenchmarkWorld extends World with HasGameReference<DissolveBenchmark> {}

class DissolveBenchmarkScreen extends StatefulWidget {
  const DissolveBenchmarkScreen({super.key});

  @override
  State<DissolveBenchmarkScreen> createState() =>
      _DissolveBenchmarkScreenState();
}

class _DissolveBenchmarkScreenState extends State<DissolveBenchmarkScreen> {
  late final DissolveBenchmark game;

  @override
  void initState() {
    super.initState();
    game = DissolveBenchmark();
  }
